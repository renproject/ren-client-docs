...
**RenJS** is the official JavaScript/TypeScript library for integrating with RenVM, allowing developers to build build cross-chain applications.
...
You can now import RenJS and the handlers for each chain, and initialize them with the desired network. (See the [Ethereum](./reference/chains/ethereum) and [Solana](./reference/chains/solana) docs for how to initialize their providers and signers)
